...
st.set_page_config(layout='wide', page_icon=r"C:\Users\ABC\Downloads\youtube-logo-png-31812.png")

# Title

#st.title('# :red[_Youtube Data Harvesting_]')
st.markdown("# :red[_Youtube Data Harvesting App_]")
st.markdown("""This web app is designed to fetch data from YouTube using their YouTube's Data API v3 based on user input for a channel username. The fetched data is stored in my personal MongoDB Compass Database and provides facilities for users to migrate the channel data to their local MySQL database. The project also includes custom-made queries for basic data analysis and visualization using Streamlit.""")
...
with st.container():
    ...
    if Get_data or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyBVntW0AFjCEZ7KmnlpgdR2-9bSNtxLw1U'
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        engine = create_engine('mysql+pymysql://root:aditya1234@localhost:3306/youtube', echo=False)
        #engine = create_engine('mysql+mysql.connector@localhost/youtube', echo=False)
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='aditya1234', db='youtube')
